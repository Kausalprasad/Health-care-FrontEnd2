...
const ProfileSetupStep2 = ({ navigation }) => {
  ...
  // Toggle dropdown for specific contact
  ...
  // Select relationship and close dropdown
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/profile/step2`, {
        method: 'POST',
        headers: {
          ...
          <View style={styles.dropdownMenu}>
            {['Spouse', 'Child', 'Parent', 'Sibling', 'Friend', 'Caregiver', 'Other'].map((option, optionIndex) => (
              <TouchableOpacity
                ...
                style={[
                  styles.dropdownItem,
                  optionIndex === 6 && styles.lastDropdownItem // Remove border from last item
                ]}
